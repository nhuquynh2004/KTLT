    ...
    def select_buttonCtl(self,id):
        ...
        while self.verticalLayoutProduct.count():
            ...
            if (widget := item.widget()):
                widget.deleteLater()
        self.update_product_layout(Catalog_info)
    def update_product_layout(self, Catalog_info):
        # Lấy danh sách sản phẩm từ danh mục được chọn
        products_to_display = list(Catalog_info.list_product.items())

        # Số lượng widget hiện có
        current_widget_count = len(self.existing_product_widgets)
        # Số lượng sản phẩm cần hiển thị
        required_widget_count = len(products_to_display)

        # Tái sử dụng các widget hiện có
        for i in range(min(current_widget_count, required_widget_count)):
        ...
        # Nếu cần thêm widget mới
        if required_widget_count > current_widget_count:
            for i in range(current_widget_count, required_widget_count):
                pdt_id, data = products_to_display[i]
                btn = QPushButton(
                    f"Product ID: {pdt_id}\nName: {data['pdt'].name}\nPrice: {data['pdt'].price}\nOrigin: {data['pdt'].origin}")
        ...
        # Ẩn các widget thừa (nếu có)
        for i in range(required_widget_count, current_widget_count):
...
#Thao tác với Product
    def select_buttonPdt(self,id):
        ...
        self.selectedProduct=self.Product[id]["btn"]
        self.selectedProduct.setChecked(True)
        Product_info=self.Product[id]["pdt"]
        ...
        if self.selectedCatalogue:
            ...
            if ctl_id in self.listCatalogue:
                if id in self.Product:
                    self.Product[id]["pdt"]=pdt
                    self.Product[id]["btn"].setText(f"Product ID: {id}\nProduct Name: {name}\nProduct Price: {price}\nProduct Origin: {origin}")
                else:
                    btn=QPushButton(f"Product ID: {id}\nProduct Name: {name}\nProduct Price: {price}\nProduct Origin: {origin}")
                    btn.setCheckable(True)
                    btn.setStyleSheet("background-color: rgb(240,237,237);text-align: left; padding-left: 10px;")
                    self.verticalLayoutProduct.addWidget(btn)
                    self.Product[id]={"pdt":pdt,"btn":btn}
                    btn.clicked.connect(lambda: self.select_buttonPdt(id))
                self.listCatalogue[ctl_id]["ctl"].list_product[id] = self.Product[id]
    ...
    def RemoveProduct(self):
        if self.selectedProduct:
            ...
            if pdt_id in self.Product:
                del self.Product[pdt_id]
    ...
    def filter_products_by_origin(self):
        ...
        # Xóa các widget hiện có trong layout
        while self.verticalLayoutProduct.count():
            item = self.verticalLayoutProduct.takeAt(0)
            if (widget := item.widget()):
                widget.deleteLater()

        # Lọc và hiển thị sản phẩm theo xuất xứ
        for catalog in self.listCatalogue.values():
            for pdt_id, product_data in catalog["ctl"].list_product.items():
                product = product_data["pdt"]
                if product.origin.lower() == origin.lower():  # So sánh không phân biệt hoa thường
                    btn = QPushButton(
